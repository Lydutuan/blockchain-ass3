...
describe("MedicalRecords Contract - Core API", function () {
  ...
  describe("checkAccess() - Check Record Access", function () {
    ...
    it("Should return false for expired access", async function () {
      // Grant access that expires in 2 seconds
  ...
  describe("Record Creation and Management", function () {
    it("Should create a new patient record", async function () {
      const ipfsCid = "QmXxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
      const tx = await medicalRecords
        .connect(admin)
        .addRecord(patient.address, ipfsCid);
      const receipt = await tx.wait();

      expect(receipt).to.not.be.null;

      const record = await medicalRecords.patientRecords(1);
    ...
    it("Should reject record creation with zero address", async function () {
      const ipfsCid = "QmXxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
      await expect(
        medicalRecords.connect(admin).addRecord(ethers.ZeroAddress, ipfsCid)
      ).to.be.revertedWith("Invalid patient address");
    });

    it("Should reject record creation with empty IPFS CID", async function () {
      await expect(
        medicalRecords.connect(admin).addRecord(patient.address, "")
      ).to.be.revertedWith("IPFS CID cannot be empty");
    });

    it("Should reject non-admin creating records", async function () {
      const ipfsCid = "QmXxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
      await expect(
        medicalRecords.connect(user).addRecord(patient.address, ipfsCid)
      ).to.be.revertedWithCustomError(medicalRecords, "AccessControlUnauthorizedAccount");
    });
    ...
    it("Should update a patient record", async function () {
      ...
      await medicalRecords
        .connect(admin)
        .addRecord(patient.address, ipfsCid1);

      const updateTx = await medicalRecords
        .connect(admin)
        .updateRecord(1, ipfsCid2);
      await updateTx.wait();

      const record = await medicalRecords.patientRecords(1);
      expect(record.ipfsCid).to.equal(ipfsCid2);
    ...
    it("Should reject updating non-existent record", async function () {
      const ipfsCid = "QmXxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
      await expect(
        medicalRecords.connect(admin).updateRecord(999, ipfsCid)
      ).to.be.revertedWith("Record does not exist");
    });
  });

  describe("Access Control with Expiry", function () {
    beforeEach(async function () {
      const ipfsCid = "QmXxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
      await medicalRecords
        .connect(admin)
    ...
    it("Should grant access to a user", async function () {
      ...
      const tx = await medicalRecords
        .connect(admin)
        .grantAccess(1, doctor.address, expiryTime, "read");
      const receipt = await tx.wait();

      expect(receipt).to.not.be.null;

      const hasAccess = await medicalRecords.hasValidAccess(1, doctor.address);
      expect(hasAccess).to.be.true;
    });
    ...
    it("Should grant access with no expiry (perpetual)", async function () {
      const tx = await medicalRecords
        .connect(admin)
        .grantAccess(1, doctor.address, 0, "read-write");
      await tx.wait();
    ...
    it("Should reject access grant with past expiry time", async function () {
      ...
      await expect(
        medicalRecords
          .connect(admin)
          .grantAccess(1, doctor.address, pastTime, "read")
      ).to.be.revertedWith("Invalid expiry time");
    });
    ...
    it("Should reject granting access to zero address", async function () {
      ...
      await expect(
        medicalRecords
          .connect(admin)
          .grantAccess(1, ethers.ZeroAddress, expiryTime, "read")
      ).to.be.revertedWith("Invalid user address");
    });

    it("Should revoke access to a user", async function () {
      const expiryTime = Math.floor(Date.now() / 1000) + 86400;
      await medicalRecords
        .connect(admin)
        .grantAccess(1, doctor.address, expiryTime, "read");

      let hasAccess = await medicalRecords.hasValidAccess(1, doctor.address);
      expect(hasAccess).to.be.true;

      const revokeTx = await medicalRecords
        .connect(admin)
        .revokeAccess(1, doctor.address);
      await revokeTx.wait();

      hasAccess = await medicalRecords.hasValidAccess(1, doctor.address);
      expect(hasAccess).to.be.false;
    });

    it("Should deny access after expiry time", async function () {
      // Grant access that expires immediately
      const expiryTime = Math.floor(Date.now() / 1000) + 2;
      await medicalRecords
        .connect(admin)
        .grantAccess(1, user.address, expiryTime, "read");

      let hasAccess = await medicalRecords.hasValidAccess(1, user.address);
      expect(hasAccess).to.be.true;
      ...
      hasAccess = await medicalRecords.hasValidAccess(1, user.address);
      expect(hasAccess).to.be.false;
    });

    it("Patient should have automatic access to own record", async function () {
      const hasAccess = await medicalRecords.hasValidAccess(1, patient.address);
      expect(hasAccess).to.be.true;
    });
  });
  ...
  describe("Record Access", function () {
    ...
    it("Should allow authorized user to read record", async function () {
      const record = await medicalRecords.connect(doctor).getRecord(1);
      expect(record.patient).to.equal(patient.address);
      expect(record.isActive).to.be.true;
    });
    ...
    it("Should deny unauthorized user from reading record", async function () {
      await expect(
        medicalRecords.connect(user).getRecord(1)
      ).to.be.revertedWith("No access to this record");
    });

    it("Should allow patient to read their own record", async function () {
      const record = await medicalRecords.connect(patient).getRecord(1);
      expect(record.patient).to.equal(patient.address);
    });
  });
  ...
  describe("Medicine Verification", function () {
    beforeEach(async function () {
      ...
      await medicalRecords
        .connect(admin)
        .grantAccess(1, doctor.address, expiryTime, "read-write");
    });

    it("Should register and verify medicine", async function () {
      const medicineName = "Aspirin";
      const qrCode = "ASPIRIN-001-QR-CODE";
      const medicineHash = ethers.id("authentic-aspirin-001");

      const tx = await medicalRecords
        .connect(doctor)
        .verifyMedicine(1, medicineName, qrCode, medicineHash);
      const receipt = await tx.wait();

      expect(receipt).to.not.be.null;

      const medicine = await medicalRecords.getMedicineDetails(1);
      expect(medicine.medicineName).to.equal(medicineName);
      expect(medicine.qrCode).to.equal(qrCode);
      expect(medicine.medicineHash).to.equal(medicineHash);
      expect(medicine.isVerified).to.be.true;
      expect(medicine.doctor).to.equal(doctor.address);
    });

    it("Should verify medicine hash authenticity", async function () {
      const medicineName = "Aspirin";
      const qrCode = "ASPIRIN-001-QR-CODE";
      const medicineHash = ethers.id("authentic-aspirin-001");
      ...
      let isValid = await medicalRecords.verifyMedicineHash(1, medicineHash);
      expect(isValid).to.be.true;

      // Wrong hash should not verify
      const wrongHash = ethers.id("fake-hash");
      isValid = await medicalRecords.verifyMedicineHash(1, wrongHash);
      expect(isValid).to.be.false;
    });

    it("Should reject medicine verification with zero hash", async function () {
      const medicineName = "Aspirin";
      const qrCode = "ASPIRIN-001-QR-CODE";
      ...
      await expect(
        medicalRecords
          .connect(doctor)
          .verifyMedicine(1, medicineName, qrCode, ethers.ZeroHash)
      ).to.be.revertedWith("Invalid medicine hash");
    });

    it("Should only allow doctors to verify medicines", async function () {
      const medicineName = "Aspirin";
      const qrCode = "ASPIRIN-001-QR-CODE";
      const medicineHash = ethers.id("authentic-aspirin-001");
    ...
    it("Should get all medicines for a record", async function () {
      const medicineName1 = "Aspirin";
      const qrCode1 = "ASPIRIN-001-QR-CODE";
      const medicineHash1 = ethers.id("authentic-aspirin-001");

      const medicineName2 = "Ibuprofen";
      const qrCode2 = "IBUPROFEN-001-QR-CODE";
      const medicineHash2 = ethers.id("authentic-ibuprofen-001");

      await medicalRecords
        .connect(doctor)
        .verifyMedicine(1, medicineName1, qrCode1, medicineHash1);

      await medicalRecords
        .connect(doctor)
        .verifyMedicine(1, medicineName2, qrCode2, medicineHash2);

      const medicines = await medicalRecords.getMedicinesForRecord(1);
      expect(medicines.length).to.equal(2);
      expect(medicines[0]).to.equal(1);
      expect(medicines[1]).to.equal(2);
    });
  });
  ...
  describe("Reentrancy Protection", function () {
    ...
    it("Should protect against reentrancy in addRecord", async function () {
      ...
      const tx1 = await medicalRecords
        .connect(admin)
        .addRecord(patient.address, ipfsCid);
      await tx1.wait();

      // Should work fine - no reentrancy issues
      expect(await medicalRecords.getRecordCount()).to.equal(2);
    });
  });
  ...
  describe("Pausable Functionality", function () {
    ...
    it("Should pause contract when called by admin", async function () {
      const pauseTx = await medicalRecords.connect(admin).pause();
      await pauseTx.wait();

      const ipfsCid = "QmYyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy";
      await expect(
        medicalRecords.connect(admin).addRecord(patient.address, ipfsCid)
      ).to.be.revertedWithCustomError(medicalRecords, "EnforcedPause");
    });

    it("Should unpause contract when called by admin", async function () {
      await medicalRecords.connect(admin).pause();
      const unpauseTx = await medicalRecords.connect(admin).unpause();
  ...
  describe("Role Management", function () {
    it("Should grant DOCTOR_ROLE", async function () {
      const DOCTOR_ROLE = await medicalRecords.DOCTOR_ROLE();
      const grantTx = await medicalRecords.grantRole(DOCTOR_ROLE, user.address);
      await grantTx.wait();

      const hasRole = await medicalRecords.hasRole(DOCTOR_ROLE, user.address);
      expect(hasRole).to.be.true;
    });

    it("Should revoke DOCTOR_ROLE", async function () {
      const DOCTOR_ROLE = await medicalRecords.DOCTOR_ROLE();
      await medicalRecords.grantRole(DOCTOR_ROLE, user.address);

      const revokeTx = await medicalRecords.revokeRole(DOCTOR_ROLE, user.address);
      await revokeTx.wait();
  ...
  describe("Query Functions", function () {
    it("Should return correct record count", async function () {
      ...
      await medicalRecords
        .connect(admin)
        .addRecord(patient.address, ipfsCid);
      expect(await medicalRecords.getRecordCount()).to.equal(1);

      await medicalRecords
        .connect(admin)
        .addRecord(user.address, ipfsCid);
      expect(await medicalRecords.getRecordCount()).to.equal(2);
    });
    ...
    it("Should return correct medicine count", async function () {
      ...
      expect(await medicalRecords.getMedicineCount()).to.equal(1);
    });
  });
  ...
  describe("Audit Log", function () {
    ...
    it("Should retrieve audit trail for a record", async function () {
      const auditTrail = await medicalRecords.getAuditTrail(1);
      expect(auditTrail.length).to.be.greaterThan(0);
    });

    it("Should get total audit trail length", async function () {
      const trailLength = await medicalRecords.getAuditTrailLength();
      expect(trailLength).to.be.greaterThan(0);
    });
  });;
